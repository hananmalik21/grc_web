...
class CreateNewInterviewNotifier extends AutoDisposeNotifier<ScheduleInterviewState>
    implements ScheduleInterviewFormActions {
  @override
  ScheduleInterviewState build() {
    return const ScheduleInterviewState(candidateGuid: '');
  }

  void setCandidate(CandidateData candidate) {
    state = state.copyWith(
      candidateGuid: candidate.id,
      fieldErrors: Map<String, String>.from(state.fieldErrors)..remove('candidate'),
  ...
  bool validate() {
    ...
    if (state.candidateGuid.trim().isEmpty) {
  ...
  Future<bool> submit() async {
    ...
    final enterpriseId = ref.read(createNewInterviewEnterpriseIdProvider);
    if (enterpriseId == null || enterpriseId <= 0) {
      state = state.copyWith(submitError: 'Select an enterprise first');
      return false;
    }

    final typeLookups = ref.read(createNewInterviewTypeLookupValuesProvider).valueOrNull ?? const [];
