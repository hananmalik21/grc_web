...
class _MobileLayout extends StatelessWidget {
  ...
  String _fmt(double v) => v == 0 ? '0h' : (v == v.roundToDouble() ? '${v.toInt()}h' : '${v.toStringAsFixed(2)}h');
...
class _MobileDayCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isRestDay = date.weekday == DateTime.friday || date.weekday == DateTime.saturday;
...
class _MobileTotalCard extends StatelessWidget {
  const _MobileTotalCard({required this.regular, required this.overtime, required this.isDark});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(10.r),
        border: Border.all(color: isDark ? AppColors.cardBorderDark : AppColors.infoText.withValues(alpha: 0.2)),
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
